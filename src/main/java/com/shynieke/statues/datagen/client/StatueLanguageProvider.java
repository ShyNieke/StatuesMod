...
public class StatueLanguageProvider extends LanguageProvider {
	...
	protected void addTranslations() {
		...
		//Config
		addConfig("general.title", "General", null);
		addConfig("statueBatSpawning", "Statue Bat Spawning", "Setting this to false disables the statue bat from spawning naturally (Default: true)");
		addConfig("ancientCityLoot", "Ancient City Loot", "When enabled this will allow Ancient City loot to contain statues (Default: true)");
		addConfig("ancientCityLootChance", "Ancient City Loot Chance", "The chance of a statue appearing in ancient City loot (Default: 0.1)");
		addConfig("statueCooldown", "Statue Cooldown", "The amount of ticks before an upgraded statue becomes interact-able (Default: 200)");
		addConfig("statueMinCooldown", "Statue Min Cooldown", "The minimum cooldown in ticks a statue can have after the speed has been subtracted (Default: 10)");
		addConfig("statueSpeedUpgrade", "Statue Speed Upgrade", "The amount of ticks subtracted from the statue cooldown per level of Speed applied (Default: 20)");

		addConfig("drops.title", "Drops", null);
		addConfig("statueDropChance", "Statue Drop Chance", "The drop chance of statues when statue drops is true (Default: 0.01)");
		addConfig("statueKillSource", "Statue Kill Source", "Source of death that determines how the Statues drop (Default: \"PLAYER\") [player statue has it's own config option],\nALL = All sources,\nPLAYER = Player only,\nPLAYER_FAKEPLAYER = Players / Fake players only");

		addConfig("player.title", "Player", null);
		addConfig("playerDropsStatue", "Player Drops Statue", "When true players will drop Player Statues when killed (Default: true)");
		addConfig("playerDropsKillSource", "Player Drops Kill Source", "Source of death that determines how the Player Statue drops (Default: \"PLAYER\") [player statue has it's own config option],\n ALL = All sources,\n PLAYER = Player only,\n PLAYER_FAKEPLAYER = Players / Fake players only");
		addConfig("playerStatueDropChance", "Player Statue Drop Chance", "The drop chance of player statues when playerDropsStatue is enabled (Default: 1.0)");
		addConfig("playerCompass", "Player Compass", "When true Statues will add a player compass (Default: true)");

		addConfig("info_messages.title", "Info Messages", null);
		addConfig("info_messages", "Info Messages", "Adding lines / removing lines specifies what the informative statue can say");

		addConfig("power_settings.title", "Power Settings", null);
		addConfig("requiresPower", "Requires Power", "When true statues will require power to function when upgraded (Default: false)");
		addConfig("powerDrain", "Power Drain", "If statues should drain power over time (Default: true)");
		addConfig("itemPowerUsage", "Item Power Usage", "The power used to generate an item (200)");
		addConfig("killPowerUsage", "Kill Power Usage", "The power used to kill a mob (500)");
		addConfig("summonPowerUsage", "Summon Power Usage", "The power used to summon a mob (500)");
		addConfig("despawnPowerUsage", "Despawn Power Usage", "The power used to despawn a mob (250)");

		addConfig("lucky_players.title", "Lucky Players", null);
		addConfig("lucky_players", "Lucky Players", "Adding usernames will make these users have less luck with getting statues");

		addConfig("client.title", "Client", null);
