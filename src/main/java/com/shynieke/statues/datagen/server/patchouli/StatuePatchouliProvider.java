...
public class StatuePatchouliProvider extends PatchouliBookProvider {
	public StatuePatchouliProvider(PackOutput output, CompletableFuture<HolderLookup.Provider> registries) {
		super(output, Reference.MOD_ID, "en_us", registries);
	}
	...
	protected void addBooks(Consumer<BookBuilder> consumer, HolderLookup.Provider provider) {
		BookBuilder bookBuilder = createBookBuilder("statues",
				"info.statues.book.name", "info.statues.book.landing", provider)
		...
		bookBuilder = bookBuilder.addCategory("info", "info.statues.book.info.name",
				...
				.addSpotlightPage(new ItemStackTemplate(StatueRegistry.STATUE_CORE.get())).build()
				.build()
				...
				.addSpotlightPage(new ItemStackTemplate(StatueRegistry.STATUE_TABLE.asItem())).build()
				.build()
