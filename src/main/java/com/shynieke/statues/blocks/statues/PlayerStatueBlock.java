				...
				if(StatuesConfig.COMMON.playerCompass.get()) {
					if(stack.getItem() == Items.COMPASS || stack.getItem() == StatueRegistry.PLAYER_COMPASS.get()) {
						...
						if(onlineFlag) {
							...
							if(player != null && player.dimension.getRegistryName().equals(playerIn.dimension.getRegistryName())) {
								...
								playerIn.sendMessage(new TranslationTextComponent("statues:player.compass.dimension.failure", TextFormatting.GOLD + playerName));
							}

						} else {
							playerIn.sendMessage(new TranslationTextComponent("statues:player.compass.offline", TextFormatting.GOLD + playerName));
						}
						return ActionResultType.SUCCESS;
					}
