...
import com.shynieke.statues.blocks.StatueBase.BlockCow;
...
public class BlockCow_Statue extends BlockCow implements ITileEntityProvider, IStatue {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		if (this.TIER >= 2) {
			if (!worldIn.isRemote) {
				...
				tile.SpecialInteraction(true, false, false, this, playerIn, worldIn, pos, hand, hitX, hitY, hitZ);
				tile.GiveItem(stack1, stack2, stack3, playerIn);
				tile.SpawnMob(new EntityCow(worldIn), worldIn);
			}
			return true;
		} else
			return false;
