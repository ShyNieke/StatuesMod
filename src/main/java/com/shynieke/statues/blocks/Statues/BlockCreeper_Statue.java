...
import com.shynieke.statues.blocks.StatueBase.BlockCreeper;
...
public class BlockCreeper_Statue extends BlockCreeper implements ITileEntityProvider, IStatue {
	...
	public BlockCreeper_Statue(String unlocalised) {
		super();
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		if (this.TIER >= 2) {
			if (!worldIn.isRemote) {
				...
				tile.PlaySound(SoundEvents.ENTITY_CREEPER_PRIMED, pos, worldIn);
				tile.GiveItem(stack1, stack2, stack3, playerIn);
				tile.SpawnMob(new EntityCreeper(worldIn), worldIn);

				tile.FakeMobs(new FakeCreeper(worldIn), worldIn, pos, false);
			}
			return true;
		} else
			return false;
