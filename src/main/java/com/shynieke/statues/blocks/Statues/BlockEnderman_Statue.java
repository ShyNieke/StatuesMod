...
import com.shynieke.statues.blocks.StatueBase.BlockHusk;
...
public class BlockEnderman_Statue extends BlockHusk implements ITileEntityProvider, IStatue {
	
	private int TIER;
	
	public BlockEnderman_Statue(String unlocalised) {
		super();
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
	{
		if(this.TIER >= 2)
		{
	        if (!worldIn.isRemote) {
	        	...
	        	tile.PlaySound(SoundEvents.ENTITY_ENDERMEN_AMBIENT, pos, worldIn);
	        	tile.GiveItem(stack1, stack2, stack3, playerIn);

	        	tile.FakeMobs(new FakeEnderman(worldIn), worldIn, pos, false);
	        }
	        return true;
		}
		else
