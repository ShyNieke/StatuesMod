...
import com.shynieke.statues.blocks.StatueBase.BlockGhast;
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		if (this.TIER >= 2) {
			if (!worldIn.isRemote) {
				...
				tile.PlaySound(SoundEvents.ENTITY_GHAST_AMBIENT, pos, worldIn);
				tile.GiveItem(stack1, stack2, stack3, playerIn);

				tile.FakeMobs(new FakeGhast(worldIn), worldIn, pos, false);
			}
			return true;
		} else
			return false;
