...
import com.shynieke.statues.blocks.StatueBase.BlockMagmaSlime;
...
public class BlockMagmaSlime_Statue extends BlockMagmaSlime implements ITileEntityProvider, IStatue {

	private int TIER;

	public BlockMagmaSlime_Statue(String unlocalised) {
		super();
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		if (this.TIER >= 2) {
			if (!worldIn.isRemote) {
				...
				tile.PlaySound(SoundEvents.ENTITY_MAGMACUBE_SQUISH, pos, worldIn);
				tile.GiveItem(stack1, stack2, stack3, playerIn);

				tile.FakeMobs(new FakeMagmaCube(worldIn), worldIn, pos, false);
			}
			return true;
		} else
			return false;
