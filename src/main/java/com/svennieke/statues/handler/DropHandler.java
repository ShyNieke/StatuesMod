	...
	public void onLivingDrop(LivingDropsEvent event) {
		...
		if(StatuesConfigGen.general.PlayersDropStatue == true)
		{
			if (entity instanceof EntityPlayer) {
				...
				{
					int random = entity.worldObj.rand.nextInt(StatuesConfigGen.general.PlayerDropChance);
					
					EntityPlayer player = (EntityPlayer) entity;
					if(random < 1)
					{
						ItemStack playerstatuestack = new ItemStack(StatuesBlocks.player_statue, 1).setStackDisplayName(player.getName());
						event.getDrops().add(new EntityItem(entity.worldObj, entity.posX, entity.posY +0.5, entity.posZ, playerstatuestack));
					}
				}
			}
		}
