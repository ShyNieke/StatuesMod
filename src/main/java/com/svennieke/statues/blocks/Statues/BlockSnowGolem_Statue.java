...
public class BlockSnowGolem_Statue extends BlockSnowGolem implements IStatue{
	
	private int TIER;
	
	public BlockSnowGolem_Statue(Block.Properties builder) {
		super(builder);
	}
	...
	public boolean onBlockActivated(IBlockState state, World worldIn, BlockPos pos, EntityPlayer playerIn, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ)
	{
		if(this.TIER >= 2)
		{
	        if (!worldIn.isRemote) {
	        	...
	        	getTE(worldIn, pos).PlaySound(SoundEvents.ENTITY_SNOW_GOLEM_AMBIENT, pos, worldIn);
	        	getTE(worldIn, pos).GiveItem(stack1, stack2, stack3, playerIn);
	        	
	        	getTE(worldIn, pos).SpawnMob(new EntitySnowman(worldIn), worldIn);
	        }
	        return true;
		}
		else
