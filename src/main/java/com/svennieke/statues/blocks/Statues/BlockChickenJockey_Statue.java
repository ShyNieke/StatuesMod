...
public class BlockChickenJockey_Statue extends BlockChickenJockey implements IStatue{
	
	private int TIER;
	
	public BlockChickenJockey_Statue(Block.Properties builder) {
		super(builder);
	...
	public boolean onBlockActivated(IBlockState state, World worldIn, BlockPos pos, EntityPlayer playerIn, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ)
	{
		if(this.TIER >= 2)
		{
	        if (!worldIn.isRemote) {
	        	...
	        	getTE(worldIn, pos).SpawnMob(new EntityZombie(worldIn), worldIn);
