...
import com.svennieke.statues.blocks.StatueBase.BlockSpider;
...
public class BlockSpider_Statue extends BlockSpider implements IStatue, ITileEntityProvider{
	
	private int TIER;
	
	public BlockSpider_Statue(String unlocalised) {
		super();
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
	{
		if(this.TIER >= 2)
		{
	        if (!worldIn.isRemote) {
	        	...
	        	getTE(worldIn, pos).PlaySound(SoundEvents.ENTITY_SPIDER_AMBIENT, pos, worldIn);
	        	getTE(worldIn, pos).GiveItem(stack1, stack2, stack3, playerIn);
	        	
	        	getTE(worldIn, pos).FakeMobs(new FakeSpider(worldIn), worldIn, pos, false);
	        }
	        return true;
		}
		else
